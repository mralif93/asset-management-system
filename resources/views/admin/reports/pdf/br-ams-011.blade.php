...
<head>
    ...
    <title>BR-AMS 011 - Senarai Rekod Aset Tak Alih</title>
    <style>
        @page {
            margin: 0;
            size: A4 landscape;
...
<body>
    <div class="header">
        <h1>BR-AMS 011 - SENARAI REKOD ASET TAK ALIH</h1>
        ...
        <tbody>
            @forelse($assets as $index => $asset)
                <tr>
                    ...
                    <td>
                        <div class="font-medium">{{ $asset->nama_aset ?? $asset->nama }}</div>
                        <div class="text-sm">{{ $asset->jenis_aset ?? '' }}</div>
                    </td>
                    <td class="text-center">{{ $asset->lokasi ?? '-' }}</td>
                    <td class="text-right font-medium amount">{{ number_format($asset->nilai_perolehan ?? $asset->kos_perolehan ?? 0, 2) }}</td>
                </tr>
            @empty
                <tr>
    ...
    <div class="note-section">
        <p><strong>ℹ️ NOTA:</strong> Laporan ini adalah senarai rasmi rekod aset tak alih.</p>
    </div>
    <div class="footer">
