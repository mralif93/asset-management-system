...
<div class="p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-300">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-sm text-yellow-600 bg-yellow-100 px-2 py-1 rounded-full font-medium">Perlu tindakan</span>
            </div>
            <h3 class="text-2xl font-bold text-gray-900 mb-1">{{ $assetMovements->where('status_pergerakan', 'menunggu_kelulusan')->count() }}</h3>
        ...
        <div class="bg-white rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-300">
            ...
            <h3 class="text-2xl font-bold text-gray-900 mb-1">{{ $assetMovements->where('status_pergerakan', 'diluluskan')->count() }}</h3>
        ...
        <div class="bg-white rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-300">
            ...
            <h3 class="text-2xl font-bold text-gray-900 mb-1">{{ $assetMovements->where('created_at', '>=', now()->startOfMonth())->count() }}</h3>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <!-- Add Movement -->
        <a href="{{ route('admin.asset-movements.create') }}" class="group bg-white rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-300">
            <div class="flex items-center justify-between mb-4">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6 mb-8 shadow-sm">
        ...
        <form method="GET" action="{{ route('admin.asset-movements.index') }}" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    <label for="jenis_pergerakan" class="block text-sm font-medium text-gray-700 mb-2">Jenis Pergerakan</label>
                    <select id="jenis_pergerakan" name="jenis_pergerakan" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                        <option value="">Semua Jenis</option>
                        <option value="Pemindahan" {{ request('jenis_pergerakan') === 'Pemindahan' ? 'selected' : '' }}>Pemindahan</option>
                        <option value="Peminjaman" {{ request('jenis_pergerakan') === 'Peminjaman' ? 'selected' : '' }}>Peminjaman</option>
                        <option value="Pulangan" {{ request('jenis_pergerakan') === 'Pulangan' ? 'selected' : '' }}>Pulangan</option>
                    </select>
                </div>
        ...
        <div class="mt-3">
            ...
            <form id="rejectForm" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea id="sebab_penolakan" 
                              name="sebab_penolakan" 
                              rows="3"
                              required
