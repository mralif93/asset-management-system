...
@section('content')
    <div class="p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <!-- Total Losses -->
            <div
                class="bg-white rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-300 hover:border-red-200">
            ...
            <!-- Pending Approvals -->
            <div
                class="bg-white rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-300 hover:border-amber-200">
                <div class="flex items-center justify-between mb-4">
            ...
            <!-- Approved Losses -->
            <div
                class="bg-white rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-300 hover:border-green-200">
                <div class="flex items-center justify-between mb-4">
            ...
            <!-- Total Loss Value -->
            <div
                class="bg-white rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-300 hover:border-purple-200">
                <div class="flex items-center justify-between mb-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <a href="{{ route('admin.loss-writeoffs.create') }}"
                class="group bg-white rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-300 hover:border-red-200">
                <div class="flex items-center justify-between mb-4">
                    <div
            ...
            <!-- Import Loss Report -->
            <a href="{{ route('admin.loss-writeoffs.import') }}"
                ...
                <p class="text-sm text-gray-600">Import rekod secara pukal</p>
            </a>

            <!-- Export Data -->
            <a href="{{ route('admin.loss-writeoffs.export', request()->query()) }}"
                onclick="openExportConfirm(event, this.href, 'Adakah anda pasti mahu eksport data kehilangan?')"
                class="group bg-white rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-300 hover:border-blue-200 cursor-pointer">
                <div class="flex items-center justify-between mb-4">
                    <div
                ...
                <p class="text-sm text-gray-600">Muat turun laporan kehilangan (.xlsx)</p>
            </a>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-8 shadow-sm">
            ...
            <form method="GET" action="{{ route('admin.loss-writeoffs.index') }}" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <!-- Status Filter -->
                    <div>
                        <label for="status_kejadian" class="block text-sm font-medium text-gray-700 mb-2">Status
                            Kejadian</label>
                        <select id="status_kejadian" name="status_kejadian"
                            ...
                            <option value="Dilaporkan" {{ request('status_kejadian') == 'Dilaporkan' ? 'selected' : '' }}>
                                Dilaporkan</option>
                            <option value="Diluluskan" {{ request('status_kejadian') == 'Diluluskan' ? 'selected' : '' }}>
                                Diluluskan</option>
                            <option value="Ditolak" {{ request('status_kejadian') == 'Ditolak' ? 'selected' : '' }}>Ditolak
                            </option>
                        </select>
                    </div>
                    ...
                    <!-- Date Filter -->
                    <div>
                        <label for="date_filter" class="block text-sm font-medium text-gray-700 mb-2">Tarikh
                            Kehilangan</label>
                        <input type="date" id="date_filter" name="date_filter" value="{{ request('date_filter') }}"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors">
                    </div>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <div class="text-sm text-gray-900">
                                        {{ $loss->tarikh_kehilangan ? $loss->tarikh_kehilangan->format('d/m/Y') : '-' }}
                                    </div>
                                    <div class="text-xs text-gray-500">
                                        {{ $loss->tarikh_kehilangan ? $loss->tarikh_kehilangan->diffForHumans() : '' }}
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-3">
                                        ...
                                        @if($loss->status_kejadian === 'Dilaporkan')
                                            <form action="{{ route('admin.loss-writeoffs.approve', $loss) }}" method="POST"
                                                class="inline">
                                                @csrf
                                                <button type="submit"
                                                    ...
                                                    title="Luluskan" onclick="return confirm('Luluskan laporan kehilangan ini?')">
                                            ...
                                            <button type="button"
                                                ...
                                                title="Tolak" onclick="openRejectModal({{ $loss->id }})">
                                ...
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 text-sm mb-6 max-w-sm">
                                            Belum ada laporan kehilangan yang didaftarkan. Mulakan dengan menambah laporan
                                            kehilangan baharu.
                                        </p>
                                        <a href="{{ route('admin.loss-writeoffs.create') }}"
        ...
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-xl bg-white">
            <div class="mt-3 text-center">
                ...
                <form id="rejectForm" method="POST" class="mt-4">
                    @csrf
                    <div class="text-left">
                        <label for="sebab_penolakan" class="block text-sm font-medium text-gray-700 mb-2">Sebab
                            Penolakan</label>
                        <textarea name="sebab_penolakan" id="sebab_penolakan" rows="3" required
